<template>
  <v-container fluid>
    <v-row>
      <h6 class="tittle-color">INFORMES PGP</h6>
      <v-col cols="6" v-for="(column, index) in visibleColumns" :key="index">
        <v-list flat class="custom-list-item">
          <v-list-item v-for="(item, itemIndex) in column" :key="itemIndex"
            @mouseover="showDescription(item.description); hoverItem = item" @mouseleave="clearDescription(); hoverItem = null"
            :href="getServiceRoute(item.route)" :style="{ '--hover-color': item.color_icon }" v-if="item.permission">
            <v-list-item-icon>
