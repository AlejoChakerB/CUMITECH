    ...
    <div class="content">
        <div class="container-fluid" style="padding: 0 0">
            <div class=" d-flex justify-content-between mb-5" style="background-color: transparent; padding: 0 0;">
                <h3 class="card-title m-0" style="font-size: 35px; color: #2B3D63;">Insumos imagenes</h3>
            </div>
            <div class="d-flex justify-content-between align-items-center flex-wrap mt-4">
                <div class="mb-2">
                    @can('create_imagingProductionSupplies')
                        <a href="{{ route('imaging.supplies') }}" class="btn btn-default buttom-left" title="Mirar reporte"
                            id="syncButton">
                            <i data-feather="refresh-cw" stroke-width="2" width="20px" height="20px"
                                id="loadContractBtn"></i><span>Actualizar precios unitarios</span>
                        </a>
                    @endcan
                </div>

                <div class="d-flex flex-wrap justify-content-start">
                    <form action="{{ route('imagingProductionSupplies.index') }}" method="GET" class="d-flex mr-2 mb-2">
                        <div class="input-group mr-2" style="max-width: 100px;">
                            ...
                            <select class="custom-select border-left-0 input" id="perPageSelect" name="per_page"
                                ...
                                <option value="10" {{ $imagingProductionSupplies->perPage() == 10 ? 'selected' : '' }}>
                                    10</option>
                                <option value="25" {{ $imagingProductionSupplies->perPage() == 25 ? 'selected' : '' }}>
                                    25</option>
                                <option value="50" {{ $imagingProductionSupplies->perPage() == 50 ? 'selected' : '' }}>
                                    50</option>
                                <option value="100" {{ $imagingProductionSupplies->perPage() == 100 ? 'selected' : '' }}>
                                    100
                                </option>
                            </select>
                        </div>
                        <div class="input-group flex-grow-1 mr-2">
                            ...
                            <input type="text" class="form-control border-left-0 input flex-grow-1" name="search"
                                placeholder="Buscar articulo" style="outline: none; box-shadow: none">
                        </div>
                    </form>

                    <div class="mb-2">
                        @can('create_imagingProductionSupplies')
                            <a href="{{ route('imagingProductionSupplies.create') }}" class="btn btn-default"
            ...
            <div class="card shadow-none border-0 mt-3">
                <div class="card-body p-0">
                    <div class="card-panel">
                        @include('imaging_production_supplies.table')
                    </div>
                </div>
            </div>
        </div>
