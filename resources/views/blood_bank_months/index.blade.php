...
<div class="content">
    <div class="container-fluid" style="padding: 0 0">
        <div class=" d-flex justify-content-between mb-5" style="background-color: transparent; padding: 0 0;">
            <h3 class="card-title m-0" style="font-size: 35px; color: #2B3D63;">Produccion banco de sangre <span>{{ $yearOnly ?? '' }}</h3>
        </div>
        <div class="d-flex justify-content-between align-items-center flex-wrap mt-4">
            <div class="mb-2">
                @can('create_bloodBankMonths')
                <a href="{{ route('blood.count') }}" class="btn btn-default buttom-left"
                ...
                @endcan
                @can('reportbi_bloodBankMonths')
                <a href="{{ route('blood_bank_months.report') }}" class="btn btn-default buttom-left" title="Mirar reporte">
                    <i data-feather="bar-chart" stroke-width="2" width="20px" height="20px"
                        id="loadContractBtn"></i><span>Reporte PowerBi</span>
                </a>
                @endcan
                @can('create_bloodBankMonths')
                <a href="{{ route('blood.calculateBlood') }}" class="btn btn-default buttom-left-cost"
            ...
            <div class="d-flex flex-wrap justify-content-start">
                <form action="{{ route('bloodBankMonths.index') }}" method="GET" class="d-flex mr-2 mb-2">
                    <div class="input-group mr-2" style="max-width: 100px;">
                        ...
                        <select class="custom-select border-left-0 input" id="perPageSelect" name="per_page"
                            ...
                            <option value="10" {{ $bloodBankMonths->perPage() == 10 ? 'selected' : '' }}>10</option>
                            <option value="25" {{ $bloodBankMonths->perPage() == 25 ? 'selected' : '' }}>25</option>
                            <option value="50" {{ $bloodBankMonths->perPage() == 50 ? 'selected' : '' }}>50</option>
                            <option value="100" {{ $bloodBankMonths->perPage() == 100 ? 'selected' : '' }}>100
                            </option>
                        </select>
                    </div>
                ...
                <div class="mb-2">
                    @can('create_bloodBankMonths')
                    <a href="{{ route('bloodBankMonths.create') }}" class="btn btn-default" title="Agregar contracto"
        ...
        <div class="card shadow-none border-0 mt-3">
            <div class="card-body p-0">
                <div class="card-panel">
                    @include('blood_bank_months.table')
                </div>
            </div>
        </div>
    </div>
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Blood Bank Months</h1>
                </div>
                <div class="col-sm-6">
                    <a class="btn btn-primary float-right"
                       href="{{ route('blood.count') }}">
                        Count
                    </a>
                    <a class="btn btn-primary float-right"
                       href="{{ route('bloodBankMonths.create') }}">
                        Add New
                    </a>
                </div>
            </div>
