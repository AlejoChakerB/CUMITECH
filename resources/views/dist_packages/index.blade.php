    ...
    <div class="content">
        <div class="container-fluid" style="padding: 0 0">
            <div class=" d-flex justify-content-between mb-5" style="background-color: transparent; padding: 0 0;">
                <h3 class="card-title m-0" style="font-size: 35px; color: #2B3D63;">Distribución paquetes</h3>
            </div>
            <div class="d-flex justify-content-between align-items-center flex-wrap mt-4">

                <div class="d-flex flex-wrap justify-content-start">
                    <form action="{{ route('distPackages.index') }}" method="GET" class="d-flex mr-2 mb-2">
                        <div class="input-group mr-2" style="max-width: 100px;">
                            ...
                            <select class="custom-select border-left-0 input" id="perPageSelect" name="per_page"
                                ...
                                <option value="10" {{ $distPackages->perPage() == 10 ? 'selected' : '' }}>10</option>
                                <option value="25" {{ $distPackages->perPage() == 25 ? 'selected' : '' }}>25</option>
                                <option value="50" {{ $distPackages->perPage() == 50 ? 'selected' : '' }}>50</option>
                                <option value="100" {{ $distPackages->perPage() == 100 ? 'selected' : '' }}>100
                                </option>
                            </select>
                        </div>
                        <div class="input-group flex-grow-1 mr-2">
                            ...
                            <input type="text" class="form-control border-left-0 input flex-grow-1" name="search"
                                placeholder="Buscar paquete" style="outline: none; box-shadow: none">
                        </div>
                    </form>
                    <div class="mb-2">
                        <a href="{{ route('detailPackages.index') }}" class="btn btn-default" title="Agregar contracto"
                            style="background-color: #2B3D63; color: white; position: relative;" id="btnAdd">
                            <div id="contentAdd" class="btn-content"><i class="fas fa-box"></i> Detalles paquetes</div>
                        </a>
                    </div>
                </div>
            </div>
