    ...
    <div class="content">
        <div class="container-fluid" style="padding: 0 0">
            <div class=" d-flex justify-content-between mb-5" style="background-color: transparent; padding: 0 0;">
                <h3 class="card-title m-0" style="font-size: 35px; color: #2B3D63;">Produccion imagenes
                    <span>{{ $yearOnly ?? '' }}</h3>
            </div>
            <div class="d-flex justify-content-between align-items-center flex-wrap mt-4">
                <div class="mb-2">
                    @can('create_imagingProductionMonths')
                        <a href="{{ route('imagingProductionMonths.count') }}" class="btn btn-default buttom-left"
                    ...
                    @endcan
                    @can('end_imagingProductionMonths')
                        <a href="" class="btn btn-default buttom-left-period" title="Calcular tarifas">
                ...
                <div class="d-flex flex-wrap justify-content-start">
                    <form action="{{ route('imagingProductionMonths.index') }}" method="GET" class="d-flex mr-2 mb-2">
                        <div class="input-group mr-2" style="max-width: 100px;">
                            ...
                            <select class="custom-select border-left-0 input" id="perPageSelect" name="per_page"
                                ...
                                <option value="10" {{ $imagingProductionMonths->perPage() == 10 ? 'selected' : '' }}>10
                                </option>
                                <option value="25" {{ $imagingProductionMonths->perPage() == 25 ? 'selected' : '' }}>25
                                </option>
                                <option value="50" {{ $imagingProductionMonths->perPage() == 50 ? 'selected' : '' }}>50
                                </option>
                                <option value="100" {{ $imagingProductionMonths->perPage() == 100 ? 'selected' : '' }}>
                                    100
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="mb-2">
                        @can('create_imagingProductionMonths')
                            <a href="{{ route('imagingProductionMonths.create') }}" class="btn btn-default"
            ...
            <div class="card shadow-none border-0 mt-3">
                <div class="card-body p-0">
                    <div class="card-panel">
                        @include('imaging_production_months.table')
                    </div>
                </div>
            </div>
        </div>
